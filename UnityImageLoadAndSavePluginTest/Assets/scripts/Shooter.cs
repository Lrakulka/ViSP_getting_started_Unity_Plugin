	...
	void Update () {
		...
		{	
			...
			if (projectile)
			{
				...
				// if the projectile does not have a render component, add one
				if (!newProjectile.GetComponent<Renderer>()) 
				{
					newProjectile.AddComponent<Renderer>();
				}
				// Set projectile color
				newProjectile.GetComponent<Renderer>().material.color = Controller.projectilesColor;
